...
tests.iosHybridTest = async function (driver, caps) {
  let el = await driver.elementByXPath("//XCUIElementTypeApplication[@name=\"WebViewsAplenty\"]/XCUIElementTypeWindow[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[1]/XCUIElementTypeTextField")
  await el.clear();
  await el.sendKeys("https://google.com");
  await driver.sleep(5);
  el = await driver.elementById("Go");
  await el.click();
  let cookies = await driver.allCookies();
  await sleep(25000);
  let found = await selectWebview(driver);
  expect(found).to.equal(true);
  el = await driver.elementByName('q');
  //let attr = await el.getAttribute('aria-label');
  //expect(attr).to.equal('Search');
  await el.sendKeys('my location');
};
...
async function androidCycle (driver, caps) {
  ...
  ["Add Contact", "Save"].should.containEql(text);
  let cb = null;
  try {
    if (appium1) {
...
tests.androidHybridTest = async function (driver) {
  let el = await driver.elementByXPath("//XCUIElementTypeApplication[@name=\"WebViewsAplenty\"]/XCUIElementTypeWindow[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[1]/XCUIElementTypeTextField")
  await el.clear();
  await el.sendKeys("https://www.7up.nl/producten/lemonlemon");
  await driver.sleep(5)
  el = await driver.elementById("Go")
  await el.click()
