...
async function runsauce (opts = null, log = true, statusFn = null) {
  ...
  if(opts.all) {
    ...
    try {
      ...
      const nativeDevices = _.filter(devices, (device) => device.api_name === "android" /*|| device.api_name === "ipad" || device.api_name === "iphone"*/);

      const tests = [];
      let androidTests = ['android_hybrid', 'android', 'web'];
      let iosTests = ['ios_hybrid', 'ios', 'web'];
      for (const {short_version:version, long_name:device, api_name} of nativeDevices) {
        const testCases = api_name === 'android' ? androidTests : iosTests;
