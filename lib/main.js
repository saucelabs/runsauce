  ...
  if(opts.all) {
    ...
    try {
      ...
      function shouldTestPlatform (platform, testCase) {
        ...
        let androidTests = ['android_hybrid', 'android', /*'android_native',*/ 'web'];
        let iosTests = ['ios_hybrid', 'ios', /*'ios_native',*/ 'web'];
        return (p === 'android' && androidTests.includes(testCase)) ||
          ((p === 'iphone' || p === 'ipad') && iosTests.includes(testCase));
      }

      // Get a list of tests to run
      let test = opts.tests[0].test;
      let testCases = test.toLowerCase().split(',');
